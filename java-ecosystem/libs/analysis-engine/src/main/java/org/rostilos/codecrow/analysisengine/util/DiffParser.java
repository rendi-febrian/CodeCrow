...
/**
 * Utility for parsing unified diff format to extract changed files and code
 * snippets.
...
public class DiffParser {
    ...
    private static final Pattern FUNCTION_PATTERN_JAVA = Pattern
            .compile("^\\s*(public|private|protected)?\\s*(static)?\\s*\\w+\\s+\\w+\\s*\\([^)]*\\)");
    private static final Pattern FUNCTION_PATTERN_PYTHON = Pattern.compile("^\\s*def\\s+\\w+\\s*\\([^)]*\\)");
    private static final Pattern FUNCTION_PATTERN_JS = Pattern
            .compile("^\\s*(function\\s+\\w+\\s*\\([^)]*\\)|const\\s+\\w+\\s*=\\s*\\([^)]*\\)\\s*=>)");
    ...
    /**
     * Extract deleted file paths from diff.
     * These are files that exist on the target branch but are being removed in the
     * PR.
     * Used to filter RAG context and inform the LLM about deletions.
     */
    public static List<String> extractDeletedFiles(String rawDiff) {
        List<DiffFileInfo> diffFiles = parseDiff(rawDiff, 0);
        List<String> paths = new ArrayList<>();
        for (DiffFileInfo file : diffFiles) {
            if ("deleted".equals(file.getChangeType())) {
