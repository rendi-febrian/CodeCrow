...
#   3. Creates deployment/.env with DB credentials + shared docker-compose secrets
#
...
main() {
  ...
  header "Step 1/2 — Copy sample config files"
  ...
  set_value "$JAVA_PROPS" "codecrow.rag.api.secret" "$INTERNAL_SECRET"
  set_value "$ORCH_ENV"   "SERVICE_SECRET" "$INTERNAL_SECRET"
  set_value "$RAG_ENV"    "SERVICE_SECRET" "$INTERNAL_SECRET"
...
EOF
  success "Internal service secret (synced across Java, inference, RAG, and .env)"
