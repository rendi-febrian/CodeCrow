...
def _cap_stage1_core_batches(
    batches: List[List[Dict[str, Any]]],
    prepared_context: Stage1PreparedContext,
    max_batches: int,
) -> List[List[Dict[str, Any]]]:
    """Admit dependency/priority-first core batches and diagnose omissions."""
    ...
    def batch_rank(index_batch: tuple[int, List[Dict[str, Any]]]) -> tuple:
        index, batch = index_batch
        relationship_rank = 0 if any(
            item.get("has_relationships") or item.get("related_files")
            for item in batch
        ) else 1
        priority = min(
            (
        ...
        return relationship_rank, priority, index

    ranked = sorted(enumerate(batches), key=batch_rank)
    selected_indices = {index for index, _ in ranked[:max_batches]}
    selected = [
    ...
    omitted_items = [item for batch in omitted for item in batch]
    omitted_paths = tuple(sorted({
        normalize_repository_path(getattr(item.get("file"), "path", ""))
        for item in omitted_items
        if normalize_repository_path(getattr(item.get("file"), "path", ""))
    }))
    omitted_hunks = tuple(sorted({
        hunk_id
        for item in omitted_items
        for hunk_id in _all_stage1_hunk_ids(item, prepared_context)
    }))
    omitted_context_chars = 0
    for item in omitted_items:
        path = getattr(item.get("file"), "path", "")
        diff_text, _, _ = _exact_stage1_diff(item, prepared_context)
        source_text = str(
            _lookup_by_path(prepared_context.file_content_by_path, path) or ""
        )
        omitted_context_chars += len(diff_text) + len(source_text)

    diagnostic = (
        "[CodeCrow Stage 1 review-wide invocation ceiling reached: "
        f"omitted_batches={len(omitted)}, omitted_paths={len(omitted_paths)}, "
        f"omitted_hunks={len(omitted_hunks)}, "
        f"omitted_context_chars={omitted_context_chars}. Dependency-bearing "
        "and higher-priority batches were admitted first. Omitted evidence is "
        "not negative evidence.]"
    )
    owner = selected[-1][-1]
    owner["_omitted_stage1_unit_count"] = (
        int(owner.get("_omitted_stage1_unit_count", 0) or 0)
        + len(omitted)
    )
    owner["_omitted_hunk_ids"] = tuple(sorted({
        *(owner.get("_omitted_hunk_ids", ()) or ()),
        *omitted_hunks,
    }))
    owner["_omitted_stage1_paths"] = tuple(sorted({
        *(owner.get("_omitted_stage1_paths", ()) or ()),
        *omitted_paths,
    }))
    owner["_omitted_context_chars"] = (
        int(owner.get("_omitted_context_chars", 0) or 0)
        + omitted_context_chars
    )
    owner["_stage1_budget_diagnostic"] = diagnostic
    logger.warning(
        "Stage 1 review-wide invocation ceiling reached: admitted_batches=%d "
        "omitted_batches=%d omitted_paths=%d omitted_hunks=%d "
        "omitted_context_chars=%d",
        len(selected),
        len(omitted),
        len(omitted_paths),
        len(omitted_hunks),
