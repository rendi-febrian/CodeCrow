...
class TestIterRepositoryFilesExtended:
    ...
    def test_custom_file_size_ceiling_is_applied(self, tmp_path):
...
class TestLoadFromDirectory:
    ...
    def test_oversized_file_is_not_loaded_partially(self, tmp_path):
        loader = DocumentLoader(RAGConfig(max_file_size_bytes=10))
        (tmp_path / "big.py").write_bytes(b"x" * 11)
        (tmp_path / "small.py").write_text("y")

        docs = loader.load_from_directory(tmp_path, "ws", "proj", "main", "abc")
        documents_by_path = {document.metadata["path"]: document for document in docs}
        assert set(documents_by_path) == {"small.py"}
